...
## 功能
- Dashboard（首頁總覽：淨資產、資產／負債概況、房貸概況、財務健康分析、資產／負債配置圓餅圖）
- 資產管理
...
- 每月生活費
- 財務目標（Goals）
- 財務健康分析（資產負債比／負債比／儲蓄率／緊急預備金月數／房貸負擔率，含規則式建議）
- 資產配置分析（占比列表＋圓餅圖）
- 開發測試模式（Developer Tools，僅供開發／驗收使用）

## 使用的外部函式庫
- [Chart.js](https://www.chartjs.org/)（v4，透過 CDN 載入）：用於首頁資產／負債配置圓餅圖，深色主題客製
